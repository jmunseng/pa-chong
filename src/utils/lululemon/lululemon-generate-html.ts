...
import { LululemonRemovedProduct } from '../../types/lululemon-product';
import { LululemonProduct } from '../../types/lululemon-product';

/**
 * 生成 N HTML 内容
 * @param e_brandSite - 品牌选项
 * @param products - 产品列表
...
export function generateLululemonHTMLContent(
    ...
    <title>${e_brandSite} Extra Sale Products</title>
    <style>
        * {
        margin: 0;
        padding: 0;
    ...
    <div class="container">
					...
					(p, i) => `                    <div class="row${
                        ...
                        <div class="cell">
                            <div class="price-info">
                                ${p.isPriceDropped || p.isPriceIncreased ? `<div class="previous-price">${p.previousPrice?.toLocaleString('ko-KR') || 0} 원</div>` : ''}
                                <div>
                                    <div class="price-krw">${p.price.toLocaleString('ko-KR')} 원</div>
                                    ${
										p.isPriceDropped
											? `<span class="price-drop-badge">降价!</span><span class="price-gap">-${p.priceGap?.toLocaleString('ko-KR') || 0} 원</span>`
											: p.isPriceIncreased
												? `<span class="price-increase-badge">涨价!</span><span class="price-gap increase">+${p.priceGap?.toLocaleString('ko-KR') || 0} 원</span>`
												: ''
									}
                                </div>
                            </div>
        ...
        <div class="removed-section">
            ...
            <ul>
        ${removedProducts.map((p) => `                <li>${p.code} - ${p.name} - ${p.price.toLocaleString('ko-KR')} 원</li>`).join('\n')}
            </ul>
        </div>`
				: ''
		}
