...
import type { LululemonProduct, LululemonProductData, LululemonRemovedProduct } from '../../types/lululemon-product';

import { E_BrandSite } from '../../enum/enum-brand-site';
import { getFilePath } from '../common';
import { generateLululemonHTMLContent } from './lululemon-generate-html';

export function comparePriceLululemon(
	e_brandSite: E_BrandSite,
	e_brandOption: null,
	previousProductData: LululemonProductData | null = null,
	currentProductData: LululemonProductData,
	fileName: string,
	prevFileName: string
) {
	if (previousProductData) {
		...
		Object.entries(currentProductData.products).forEach(([code, product], index) => {
			...
			if (!previousProductInfo) {
				...
				// console.log(`✓ 新产品: ${code} - ${product.name}: ${currentPrice.toLocaleString()} 원`);
				...
				priceDropCount++;
				// console.log(
				// 	`✓ 价格下降: ${code} - ${product.name}: ${previousPrice.toLocaleString()} → ${currentPrice.toLocaleString()} (降了 ${product.priceGap.toLocaleString()} 원)`
				// );
			} else if (previousPrice && currentPrice > previousPrice) {
				// 价格上涨
				...
				// console.log(
				// 	`✓ 价格上涨: ${code} - ${product.name}: ${previousPrice.toLocaleString()} → ${currentPrice.toLocaleString()} (涨了 ${product.priceGap.toLocaleString()} 원)`
				// );
			}
		...
		// 查找已下架的产品
		const removedProducts: LululemonRemovedProduct[] = [];
		...
		// 重新生成HTML，包含价格比较信息
		const htmlContentWithComparison = generateLululemonHTMLContent(
			e_brandSite,
			uniqueProducts,
			dateTimeString,
		...
		const htmlFilePathAndName = getFilePath(e_brandSite, null, fileName, 'html');
		...
		const uniqueProducts = Object.values(currentProductData.products);
		const dateTimeString = currentProductData.dateTimeString;
		const htmlContent = generateLululemonHTMLContent(e_brandSite, uniqueProducts, dateTimeString);
		const htmlFilePathAndName = getFilePath(e_brandSite, null, fileName, 'html');
