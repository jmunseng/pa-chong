...
import type { NikeProduct, NikeProductData, RemovedProduct } from '../../types/nike-product';

import { E_BrandSite } from '../../enum/enum-brand-site';
import { getFilePath } from '../common';
import { generateNikeHTMLContent } from './nike-generate-html';
...
export function comparePriceNike(
	...
	if (previousProductData) {
		...
		Object.entries(previousProductData.products).forEach(([code, productInfo]) => {
			if (!currentCodes.has(code)) {
				removedProducts.push({
					...
					goodsLinkUrl: productInfo.url,
					thumbnail: productInfo.imageUrl,
				});
		...
		// 重新生成HTML，包含价格比较信息
		const htmlContentWithComparison = generateNikeHTMLContent(
			e_brandSite,
			uniqueProducts,
			dateTimeString,
		...
		const uniqueProducts = Object.values(currentProductData.products);
		const dateTimeString = currentProductData.dateTimeString;
		const htmlContent = generateNikeHTMLContent(e_brandSite, uniqueProducts, dateTimeString);
		const htmlFilePathAndName = getFilePath(e_brandSite, null, fileName, 'html');
